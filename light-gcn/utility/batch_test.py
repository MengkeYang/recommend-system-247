...
from evaluator import eval_score_matrix_foldout, save_ranking
...
def predict(sess, model, users_to_test, drop_flag=False, train_set_flag=0):
    top_show = np.sort(model.Ks)
    max_top = max(top_show)
    ...
    for u_batch_id in trange(n_user_batchs):
        ...
        save_ranking(rate_batch, test_items, f_pred, max_top)
